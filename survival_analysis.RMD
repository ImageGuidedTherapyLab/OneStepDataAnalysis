...
if( is.null(status) || !(status %in% names(dat))) {
  status <- "status"
  dat[status] <- 1
...
regressionTrainingRows     <- dat[,status]==1 
...
corrplot(cor(Filter(is.numeric,dat[variableSelections$multivariate]), use="pairwise"),
   ...
   mar = c(1,2,2,0),
   order = "hclust" )
} else if ( length(variableSelections$multivariate) == 0 ){
   variableSelections$multivariate <- NULL
}
...
if( !is.null(variableSelections$multivariate) ){
...
modelsClass[[model_name]] <- do.call(caret::train, c(caretparams[[iii]], list(data=modeldata)))  

#TODO: fix weights
#, weights=weightList


#Sort $pred data frame by row index
modelsClass[[model_name]]$pred <- modelsClass[[model_name]]$pred[order(modelsClass[[model_name]]$pred$rowIndex),]
...
accs_with_ci =data.frame( lapply(modelsClass,function(x) confusionMatrix(x$pred$pred,reference = x$pred$obs, positive = timelabs[length(timelabs)])$overall))
...
ROCdropcensor <- roc(response = resultsClass$obs, levels = timelabs, predictor = resultsClass[,timelabs[length(timelabs)]], ci=T, of="auc")
# seed the bootstrap ROC interval
set.seed(555)
spec.ci = ci.se(ROCdropcensor, specificities = seq(0,1,0.05), boot.n=500)
print(ROCdropcensor)
plot(ROCdropcensor, print.auc=T,main="Only observations used for model building")
plot(spec.ci,type="shape", col="lightgray") 
print("")
...
#use $pred$rowIndex in case pred/obs are out of order
fullClassPreds[which(classificationTrainingRows)[bestClassifier$pred$rowIndex]] = bestClassifier$pred$pred

if( any(!classificationTrainingRows)){
  fullClassPreds[-which(classificationTrainingRows)[bestClassifier$pred$rowIndex]] = predict(bestClassifier,datClass[-which(classificationTrainingRows)[bestClassifier$pred$rowIndex],]) #make predictions on censored obs
}
...
probLong[which(classificationTrainingRows)[bestClassifier$pred$rowIndex]] = bestClassifier$pred[, timelabs[2] ] #make predictions on known obs
...
ROC1 <- roc(response = datClass$survivorType, #probLong has the observations put in original order
                       levels = timelabs,
                       predictor = probLong)###

print(ROC1)
plot(ROC1, main="Includes all (i.e. censored) observations")
print("")

#Check if ROC1 reduces to ROCdropcensor if we manually take only training rows

ROC1reduced <-  roc(response = datClass$survivorType[classificationTrainingRows],
                      levels = timelabs,
                   predictor = probLong[classificationTrainingRows])

if(ROC1reduced$auc != ROCdropcensor$auc){
  stop("ROCs do not agree, check for correct ordering of predicted values")
}
...
print("Confusion Matrix for all observations (includes censored)")
...
#ENH: uses probability output instead of classification for ROC analysis
if(!is.null(scopefixed_inputs) ){
fixedClassPreds <- numeric()
fixedClassifier <- modelsClass$Class_fixed_forest
fixedClassPreds[ classificationTrainingRows] = fixedClassifier$pred[order(fixedClassifier$pred$rowIndex),timelabs[length(timelabs)]] 

if(any(!classificationTrainingRows)){
  fixedClassPreds[!classificationTrainingRows] = predict(fixedClassifier,datClass[!classificationTrainingRows,], type="prob")[,timelabs[length(timelabs)]]
}
...
print(rocFixed)
plot(rocFixed, main="random forest with fixed inputs only. All observations")
print("")
...
#Sort $pred data frame by row index
models[[model_name]]$pred <- models[[model_name]]$pred[order(models[[model_name]]$pred$rowIndex),]
...
#      )
  ...
  names(acc) <- rs$models
  maxacc <- max(apply(acc,2,mean),na.rm=TRUE)
  maxmodels <- rs$models[apply(acc,2,mean,na.rm=TRUE)==maxacc]
  bestmod <- models[[maxmodels[[1]]]] #pick first by default
...
fullPreds[which(regressionTrainingRows)[bestmod$pred$rowIndex]]  = bestmod$pred$pred #hold-out predictions on non-censored

#make predictions on censored non-responders if any
if( any(!regressionTrainingRows)){
  fullPreds[!regressionTrainingRows] = predict(bestmod,dat[!regressionTrainingRows,])
}

print("C index (Concordance) for best model: 0.5 represents random prediction. Includes censored cases")
...
if(!is.null(params$foldID)){
  ...
  datFull[rownames(dat),finalpredname] <- results$pred  #trickey line of code since results$rowIndex referes to rows of dat not datFull
    }
...
if(!is.null(params$foldID)){
  ...
  print( ifelse(is.null(status),
                kable(datFull[,c(survival,status,finalpredname)]),
                kable(datFull[,c(survival,       finalpredname)]))
       )

  # HACK: @egates1 how do you want to add this global search?
 
  if(!is.numeric(datFull[,finalpredname]) ){ #error handle for regression case
