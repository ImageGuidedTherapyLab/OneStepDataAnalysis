...
if(params$plot){
  ...
  for (varname in inputs) {
    ...
    if(!is.factor(dat[,varname])){
      ...
      plot(dat[,varname], dat[,survival], xlab=varname, ylab=survival, pch=21, bg='gray',col='black', main=sprintf("Pearson r: %0.3f",cor(dat[,varname],dat[,survival],method="pearson")))
      print(" ")
    }
  }
}
...
# Modeling does NOT include censored data
...
#take uncensored rows

if(! is.null(params$status)){
  modeldata  <- datClass[datClass[,params$status]==1,c("survivorType",variableSelections[[jjj]])]
} else {
