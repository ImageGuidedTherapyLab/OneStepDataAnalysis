...
output: pdf_document
params:
  csvPath:          /rsrch1/ip/egates1/MICCAI_BraTS_2018_Data_Training/BraTS18_pyradiomicsout_tumor_segsubvolumes_withKi67CDERG_NLM_combined_survivalonly_SHORT.csv
  outCsvName:       !r NUMM
  survival:         survival
  status:           !r NULL 
  regexp:           !r "(_Volume|firstorder|tumor_original_shape_Maximum|Sphericity)"
  fixed_inputs:        age
  exclude_inputs:      "(Energy|Entropy|AbsoluteDeviation)"  
  hierarch:         !r FALSE
  nfold:               5
  leaveOneOut:      !r TRUE 
  foldID:              1
...
libs <- c("corrplot", "survival", "ggplot2", "caret", "knitr", "MASS", "Boruta", "gridExtra", "randomForest","reshape2","glmnet")
...
```{r Input Histograms, echo=FALSE, warning=FALSE, message=FALSE, fig.width=3, fig.height=3}
...
## Survival Modeling:
 ...
 Bouta method (with correlation reduction) can be used with teh _boruta_ parameter.

```{r Univariate Filter, echo=FALSE, warning=FALSE, message=FALSE, fig.width=6, fig.height=7}

mybreaks = params$breaks

if( any(params$breaks %in% dat[,survival]) ){warning("One survival time is exactly coincident with a break value, this will create a NA")}
...
survivorType = cut(as.matrix(dat[survival]), breaks = params$breaks,
                   labels=timelabs
                  )
...
variableSelections <- list(fixed=params$fixed_inputs)
...
variableSelections$coxunivariate <- union(params$fixed_inputs, setdiff(inputs, inputs[univPvals > 0.05]) )#discard variables above threshold
...
# Might have a bug with zv and corr...
...
variableSelections$univariate <- union(nums[pvals < pvalThresh], params$fixed_inputs) #discard variables above threshold
...
print(coxMultiModel)
#TODO: see if inline print statement between blocks prints this too
...
print(survConcordance(survObj~as.numeric(resultsClass$pred)))
...
dat$predSurvivorType <- bestClassifier$pred$pred
...
print(survConcordance(survObj ~ results$pred))
...
correlation cutoff: `params$cutoff`
...
# Predictions on validation set using the holdout and teh best classifiers and 
...
kable(data.frame(Actual=validationdatReduced[,survival],PredictedClassContinuous=predType, ActualClass=realClass, PredictedClassClassifier=validationdat$predSurvivorType))
...
predname <- paste("validationPreds",params$foldID,sep="_")
...
if(is.null(params$outCsvName)){
  if(!is.null(params$foldID)){
    outCsv <- sub(".csv",sprintf("_Fold%s_validation.csv",params$foldID),params$csvPath) 
  } else {
...
#TODO: Write CV predictions if no validation set
