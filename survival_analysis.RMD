...
pvals <- lapply(nums,
       function(var) {          
           test <- cor.test(dat[dat[,status]==1 | datClass$survivorType == timelabs[length(timelabs)], c(var)], dat[dat[,status]==1 | datClass$survivorType == timelabs[length(timelabs)], survival])
...
corrplot(cor(Filter(is.numeric,dat[dat[,status]==1 | datClass$survivorType == timelabs[length(timelabs)],variableSelections$univariate]), use="pairwise"),
...
if(!is.null(params$foldID)){
  ...
  datFull[trainingpredname] <- NA
  datFull[rownames(dat),trainingpredname] <- results$pred  #trickey line of code since results$rowIndex referes to rows of dat not datFull
}
