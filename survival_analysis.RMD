...
print(caret::confusionMatrix( ifelse( resultsClass$long < maxthreshold , timelabs[1],timelabs[length(timelabs)]), reference = resultsClass$obs, positive = timelabs[length(timelabs)]) )
...
print(caret::confusionMatrix( ifelse( probLong < maxthreshold , timelabs[1],timelabs[length(timelabs)]), reference = datClass$survivorType,  positive = timelabs[length(timelabs)]) )

}

#  use RF on fixed variables as reference control
if(!is.null(scopefixed_inputs) ){
fixedClassPreds <- factor(levels = levels(datClass$survivorType))
fixedClassifier <- modelsClass$Class_fixed_forest
fixedClassPreds[which(datClass[,status]!= 0)] = fixedClassifier$pred$pred 
fixedClassPreds[which(datClass[,status]== 0)] = predict(fixedClassifier,datClass[datClass[status] != 1,])
}
