...
# @egates1 - can you overlap two histograms that separate the censored data?
#  overlapping histograms are hard to read/interpret
...
if(length(Filteredinputs)>1){
  pp <- preProcess(dat[dat[,status]==1 | datClass$survivorType == timelabs[length(timelabs)],Filteredinputs], method = ppMethods, cutoff=params$cutoff)
...
bor <- Boruta(x=datClass[dat[,status]==1 | datClass$survivorType == timelabs[length(timelabs)], borutaInputs ], y=datClass[dat[,status]==1 | datClass$survivorType == timelabs[length(timelabs)], "survivorType"], pValue=.35)
...
correlations <- cor(Filter(is.numeric,dat[dat[,status]==1 | datClass$survivorType == timelabs[length(timelabs)],variableSelections$boruta]), use="pairwise")
...
if(! is.null(params$status)){
  modeldata  <- datClass[ (datClass[,status]==1 | datClass$survivorType == timelabs[length(timelabs)] ), c("survivorType",variableSelections[[jjj]])]
  weightList <- datClass[  (datClass[,status]==1 | datClass$survivorType == timelabs[length(timelabs)] ), trainingWeights]
} else {
...
invisible(  modelsClass[[model_name]] <- do.call(caret::train, c(caretparams[[iii]], list(data=modeldata, weights=weightList)))  )
...
if(! is.null(params$status) ){
  modeldata  <- dat[   dat[,status]==1 | datClass$survivorType == timelabs[length(timelabs)]  , c(survival,variableSelections[[jjj]])]
  weightList <- dat[  (dat[,status]==1 | datClass$survivorType == timelabs[length(timelabs)] ), trainingWeights]
} else {
  modeldata  <- dat[   dat[,status]==1 | datClass$survivorType == timelabs[length(timelabs)]  , c(survival,variableSelections[[jjj]])]
  weightList <- dat[  (dat[,status]==1 | datClass$survivorType == timelabs[length(timelabs)] ), trainingWeights]
}
...
print(model_name)
set.seed(3141) #seed before train to get same subsamples
invisible(  models[[model_name]] <- do.call(caret::train, c(caretparams[[iii]], list(data=modeldata, weights=weightList)))  )
...
fullPreds[dat[,status] ==1 ]  = bestmod$pred$pred #LOOCV on non-censored
#make predictions on censored non-responders
fullPreds[dat[,status] !=1] = predict(bestmod,dat[dat[status] != 1 & datClass$survivorType != timelabs[length(timelabs)],])
...
if(!is.null(params$foldID)){
  ...
  trainingpredname <- "fullPreds"
  datFull[,trainingpredname] <- NA
  datFull[rownames(dat),trainingpredname] <- fullPreds  #trickey line of code since results$rowIndex referes to rows of dat not datFull
}
...
if(!is.null(params$foldID)){
  kable(datFull[,c(survival,status,predname,trainingpredname)])
} else {
  kable(datFull[,c(survival,status,trainingpredname)])

  print(roc(datFull[,survival] < 100, datFull[,trainingpredname] ))
  plot(roc(datFull[,survival] < 100, datFull[,trainingpredname] ))
# @egates1 need precision recall curves
# https://towardsdatascience.com/what-metrics-should-we-use-on-imbalanced-data-set-precision-recall-roc-e2e79252aeba
#  Using packace "PPROC"

#May give "NA" if NAs are present, currently hacked to remove NA predictions
PR1 <- pr.curve(weights.class0=as.numeric(datFull[!is.na(datFull[,trainingpredname]),survival] < 100), scores.class0=na.omit(datFull[,trainingpredname]), curve=T)
