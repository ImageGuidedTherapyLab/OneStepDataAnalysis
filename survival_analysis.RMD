...
if(params$plot){
  ...
  for (varname in inputs) {
    ...
    if(!is.factor(dat[,varname])){
      ...
      hist(dat[dat[,status]==1,varname], col="gray", main = paste("Max in row:", maxrow), xlab=varname)
...
fullClassPreds <- numeric(nrow(survObj))

fullClassPreds[which(datClass[,status]!= 0)] = bestClassifier$pred$pred
fullClassPreds[which(datClass[,status]== 0)] = predict(bestClassifier$finalModel,datClass[datClass[status] != 1,]) #make predictions on censored obs
